biling.survey = function(N = 40, pBi = .5, ...){
  ...
  par(mgp = c(2.5, .5, 0), las = 1, tck = -.02, font.lab = 2, cex.lab = 1, ...)
  
  plot.ts(runProp, ty = "o", ylim = c(0, 1), yaxt = "n", pch = 21, bg = 3, xlab = "Number of Respondents", ylab = "Proportion of (B)", cex = .9)
  
  axis(2, at = seq(0, 1L, len = 6), labels = paste0(seq(0, 1e2, len = 6), "%"))
  
  ResponseLetters = paste( c("M","B")[ResponseSequence[1L:1e1] + 1], collapse ="" )    # Display the first 10 response sequence
