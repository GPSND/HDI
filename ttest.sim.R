...
ttest = function(n1 = 5, n2 = 5, 
  ...
  ttest.sim = function(){
    ...
    if(!researcher.tool){
     ...
     # mean.g1 = sample( seq(mean + .1*mean, mean + .5*mean, len = 1e2), 1, replace = TRUE)   #sample(runif(1e3, min, max), 1, replace = TRUE) 
     # mean.g2 = if(null.effect) mean.g1 else sample( seq(mean, mean + .3*mean, len = 1e2), 1, replace = TRUE)       #sample(rnorm(1e3, min + 3, max), 1, replace = TRUE) 
      #sd = sample( seq(1.2*sd, 1.6*sd, len = 1e2), 1, replace = TRUE)          #sample( runif(1e2, min = 1.2*sd, max = 1.6*sd), 1, replace = TRUE) #sample(runif(1e3, 0, 10), 1) 
    ...
    if(paired & !researcher.tool){
      ...
      library(MASS)
      mvnorm.mat <- mvrnorm(n1, Sigma = cov.pop, mu = mu) # by default: empirical = FALSE
...
ttest(n1 = 30, n2 = 30, sim.time = .7, min.score = 0, max.score = 20, 
      simulation = FALSE, n.sim = 2, paired = F, subjects = F, descriptives = T,
      lcolor = "gray70", researcher.tool = TRUE)
